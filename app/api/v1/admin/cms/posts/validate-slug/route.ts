...
export const GET = adminApiWrapper(async (request: NextRequest, adminUser) => {
  ...
  const response = await withDatabaseOperation(
    async () => {
      const authContext = {
        userId: adminUser.id,
        operation: 'validate_cms_post_slug',
        reason: 'Admin validating CMS post slug uniqueness for content management',
        source: 'admin/cms/posts/validate-slug',
        metadata: {
          endpoint: '/api/v1/admin/cms/posts/validate-slug',
          slug,
          excludeId
        }
      }
      ...
      const data = await SecureServiceRoleWrapper.executeSecureOperation(
        ...
        async () => {
          ...
          // Exclude specific post ID when checking for updates
          if (excludeId) {
    ...
    { userId: adminUser.id, endpoint: '/api/v1/admin/cms/posts/validate-slug' }
  )
